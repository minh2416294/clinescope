...
All four are real Cline CLI (World-A) traces, scored with `read_files apply_patch` as the
expected tools.
...
The four traces are pinned by `tests/test_harness_gap_capture.py` (skipif-gated on their
