...
`clinescope-gate` gates on. Clinescope also ships one optional LLM judge (used by `diff_minimality` to catch subtler "wasteful edit" cases that the deterministic heuristic misses).
